...
console.log("Đang truy vấn Supabase REST API tối ưu...");

// Chỉ lấy các trường cần thiết và lọc success=false, không order để tránh table scan nặng
const url = `${supabaseUrl}/rest/v1/ai_logs?select=id,success,error_message,created_at,model_id,action_type,input_preview,output_preview&success=eq.false&limit=5`;
...
.then(data => {
    console.log(`Thành công! Tìm thấy ${data.length} logs lỗi.`);
