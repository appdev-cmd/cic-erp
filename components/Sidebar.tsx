...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const managementItems = NAV_ITEMS.filter(item => ['dashboard', 'tasks', 'projects', 'contracts', 'payments', 'analytics', 'documents', 'reports', 'ai-assistant', 'agent-manager', 'tools'].includes(item.id) && !hiddenItems.has(item.id) && (!item.devOnly || isDevLocal));
  const categoryItems = NAV_ITEMS.filter(item => ['units', 'personnel', 'hrm', 'products', 'customers', 'user-guide'].includes(item.id) && !hiddenItems.has(item.id) && (!item.devOnly || isDevLocal));
