...
import { Search, Filter, Plus, Building2, Phone, Mail, MoreVertical } from 'lucide-react';
...
export const CompaniesPage: React.FC = () => {
  ...
  const companies = [
    { id: '1', name: 'Công ty Cổ phần Đầu tư CIC', phone: '024.1234.5678', email: 'contact@cic.com.vn', industry: 'Xây dựng', address: 'Hà Nội' },
    { id: '2', name: 'Tập đoàn Công nghệ Giga', phone: '028.9876.5432', email: 'info@giga.vn', industry: 'Công nghệ', address: 'TP. Hồ Chí Minh' }
  ];
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 uppercase">
                  <tr>
                    <th className="px-6 py-4 font-medium">Tên Công Ty</th>
                  ...
                  {companies.map((company) => (
                    <tr key={company.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-700 dark:text-slate-300">
                        <span className="flex items-center gap-1.5"><Phone className="w-3.5 h-3.5 text-slate-400" /> {company.phone}</span>
                      </td>
                      <td className="px-6 py-4 text-slate-700 dark:text-slate-300">
                        <span className="flex items-center gap-1.5"><Mail className="w-3.5 h-3.5 text-slate-400" /> {company.email}</span>
                      </td>
                      <td className="px-6 py-4">
                        <span className="px-2.5 py-1 text-xs font-semibold rounded bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400">
                          {company.industry}
                        </span>
                      </td>
