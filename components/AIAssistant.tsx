...
// ─── Lazy Loaded AI Pages ──────────────────────────────────────────────────
const AgentManager = React.lazy(() => import('./AgentManager'));
const AIPermissionManager = React.lazy(() => import('./settings/AIPermissionManager'));
...
const KNOWN_WORKING_LOCAL_MODELS = ['qwen3.5-35b'];
...
const AIAssistant: React.FC = () => {
  ...
  const [currentModel, setCurrentModel] = useState<string>(() => {
    ...
    const staleModels = ['Qwen3.5', 'gemma-3-9b', 'gemma-2-9b', 'Qwen2.5-14B', 'qwen2.5-14b', 'cic-legal-14b', 'qwen-2.5-14b', 'qwen2.5-72b', 'gemma-4-26b', 'qwen2.5-7b', 'qwen2.5-vl-7b', 'qwen2.5-32b'];
    if (saved && staleModels.some(s => saved.includes(s))) {
      localStorage.removeItem(MODEL_STORAGE_KEY);
      return 'qwen3.5-35b';
    }
    return saved || 'qwen3.5-35b';
  });
  
  const [onlineLocalModels, setOnlineLocalModels] = useState<string[]>(['qwen3.5-35b']);
  ...
  const [activeTab, setActiveTab] = useState<AITab>(() => (localStorage.getItem('cic-ai-active-tab') as AITab) || 'chat');
  const handleTabChange = (tab: AITab) => {
    setActiveTab(tab);
    localStorage.setItem('cic-ai-active-tab', tab);
  };
  ...
  const testLocalAI = useCallback(async () => {
    ...
    try {
      ...
      if (models.length > 0) {
        // Lấy toàn bộ các model trả về từ máy chủ, loại bỏ các model embedding
        const activeModels = models.filter(m => !m.toLowerCase().includes('embed') && !m.toLowerCase().includes('rerank')).sort();
        ...
        if (activeModels.length > 0) {
          ...
          const isLocalModel = currentModel.startsWith('qwen') || currentModel.startsWith('gemma') || currentModel.startsWith('llama') || currentModel.startsWith('mistral');
          ...
          throw new Error('Không có mô hình nào đang hoạt động thực tế trên máy chủ');
        }
      } else {
  ...
  // Đảm bảo nhân viên thường không sử dụng mô hình Cloud trả phí
  useEffect(() => {
    if (_profile) {
      const isAdminOrDev = ['Admin', 'Leadership'].includes(_profile.role || '');
      if (!isAdminOrDev && !onlineLocalModels.includes(currentModel)) {
        setCurrentModel(onlineLocalModels[0] || 'qwen3.5-35b');
      }
    }
  }, [_profile, currentModel, onlineLocalModels]);
  ...
  return (
    <div className={cn(
      "flex h-full w-full bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 transition-all duration-300 overflow-hidden relative",
          ...
          <div className="flex items-center gap-3 min-w-0">
            <div className="flex items-center gap-3 min-w-0">
              ...
              <div className="min-w-0">
                <h3 className="font-bold text-slate-800 dark:text-slate-100 text-sm flex items-center gap-1.5 leading-none mb-1">
                  AI Agent Assistant
            ...
            <>
                ...
                <div className="hidden md:block mr-1">
                  <select
                    ...
                    className="bg-slate-100 dark:bg-slate-800 hover:bg-slate-200 dark:hover:bg-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-300 py-1.5 px-2 rounded-lg cursor-pointer focus:outline-none border border-slate-200 dark:border-slate-800 transition-all"
                    title="Chọn Model AI"
                  >
              ...
              <div className="divide-y divide-slate-100 dark:divide-slate-800/40">
                {conversations.map(conv => (
                  <button
                    ...
                    className={cn(
                      "w-full text-left px-4 py-2 text-xs transition-colors cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 flex items-start gap-3 group",
        ...
        <div className="flex-1 overflow-hidden relative">
          ...
          {activeTab === 'chat' && (
            <div className="flex flex-col h-full overflow-hidden">
              ...
              {widgetHistoryBanner && (
                <div className="mx-4 md:mx-6 mt-4 p-3 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-xl flex items-center gap-3 shrink-0">
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[60] bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto p-5 md:p-6 bg-slate-50 dark:bg-slate-900">
                      ...
                      <div className={cn(
                        ...
                        {localAITestResult.ok ? (
                          <>
                            ...
                            <div className="max-h-28 overflow-y-auto space-y-1 pr-2">
                              {localAITestResult.models.map((m, i) => (
                                <p key={i} className="text-[10px] text-emerald-600 dark:text-emerald-400 font-mono">• {m}</p>
                              ))}
                            </div>
                          </>
                        ) : (
                          <>
                            ...
                            <p className="text-[10px] text-rose-600 dark:text-rose-400 mt-1 leading-normal">Kiểm tra: <code className="bg-rose-100 dark:bg-rose-900/30 px-1 rounded font-mono text-[9px]">ollama serve</code> đã chạy chưa?</p>
                          </>
                        )}
                      </div>
                    )}
              ...
              {settingsTab === 'guide' && (
                <div className="space-y-4 max-h-[50vh]">
                  <div className="rounded-xl border border-blue-100 dark:border-blue-900/40 bg-blue-50/20 dark:bg-blue-950/10 p-4">
                    ...
                    <ul className="list-decimal pl-4 text-[10px] text-slate-600 dark:text-slate-400 space-y-1 font-semibold leading-relaxed">
