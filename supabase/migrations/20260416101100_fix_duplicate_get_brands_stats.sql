-- Drop both existing functions to avoid signature conflicts
DROP FUNCTION IF EXISTS public.get_brands_with_stats(text, text, text);
DROP FUNCTION IF EXISTS public.get_brands_with_stats(text, text, text, text, integer, integer);
