...
export const ContractService = {
    ...
    // ========================================================================
    // RELATED CONTRACTS (Bidirectional linking with approval flow)
    // ========================================================================

    /**
     * Get all APPROVED related contracts (both directions).
     */
    getRelatedContracts: async (contractId: string): Promise<{ id: string; contractCode: string; title: string; partyA: string; value: number; status: string; signedDate: string; unitId: string }[]> => {
        try {
            ...
            if (error) {
                console.error('[ContractService] getRelatedContracts error:', error);
                return [];
            }
            ...
            if (fetchError) {
                console.error('[ContractService] fetch related contracts error:', fetchError);
                return [];
            }
            ...
            console.error('[ContractService] getRelatedContracts exception:', e);
            return [];
        }
    },
     ...
     */
    getOutgoingPendingLinks: async (contractId: string): Promise<{ id: string; contractCode: string; title: string; partyA: string; unitId: string }[]> => {
        try {
            const { data, error } = await supabase
                .from('contract_relations')
            ...
            console.error('[ContractService] getOutgoingPendingLinks error:', e);
            return [];
        }
    },
     ...
     */
    getIncomingPendingLinks: async (contractId: string): Promise<{ id: string; contractCode: string; title: string; partyA: string; value: number; unitId: string }[]> => {
        try {
            const { data, error } = await supabase
                .from('contract_relations')
            ...
            console.error('[ContractService] getIncomingPendingLinks error:', e);
            return [];
        }
    },
     ...
     */
    linkContracts: async (contractId: string, relatedContractId: string): Promise<'approved' | 'pending' | false> => {
        try {
            ...
            if (sameUnit) {
                ...
                if (error) {
                    console.error('[ContractService] linkContracts (same-unit) error:', error);
                    return false;
                }
                return 'approved';
            } else {
                ...
                if (error) {
                    console.error('[ContractService] linkContracts (cross-unit) error:', error);
                    return false;
                }
                return 'pending';
            }
        } catch (e) {
            console.error('[ContractService] linkContracts exception:', e);
            return false;
        }
    },
     ...
     */
    approveLink: async (contractId: string, requesterContractId: string): Promise<boolean> => {
        try {
            ...
            if (e1 || e2) {
                console.error('[ContractService] approveLink error:', e1 || e2);
                return false;
            }
            return true;
        } catch (e) {
            console.error('[ContractService] approveLink exception:', e);
            return false;
        }
    },
     ...
     */
    rejectLink: async (contractId: string, requesterContractId: string): Promise<boolean> => {
        try {
            const { error } = await supabase
                .from('contract_relations')
                .delete()
            ...
            if (error) {
                console.error('[ContractService] rejectLink error:', error);
                return false;
            }
            return true;
        } catch (e) {
            console.error('[ContractService] rejectLink exception:', e);
            return false;
        }
    },
     ...
     */
    unlinkContracts: async (contractId: string, relatedContractId: string): Promise<boolean> => {
        try {
            const { error: e1 } = await supabase
                .from('contract_relations')
                .delete()
            ...
            if (e1 || e2) {
                console.error('[ContractService] unlinkContracts error:', e1 || e2);
                return false;
            }
            return true;
        } catch (e) {
            console.error('[ContractService] unlinkContracts exception:', e);
            return false;
        }
    },
};
